...
## 5. ⚠️ ข้อค้นพบระหว่างทำที่ต้องเขียนในเล่มอย่างซื่อสัตย์

รูป valid set เป็น **640×640 อยู่แล้ว** → ใน `bench_latency.py` การ resize เป็น no-op
49.7 ms ที่วัดได้จึง**ไม่ใช่ค่า resize** แต่คือ **numpy float path** (`astype(float32)/255*64 → round → clip → astype(int8)` บน 1.2M ค่า)
บน host x86 โหมด `lut` (cv2.LUT แทน float path) เร็วกว่า `numpy` **8.7×** โดยผลตรงกันทุกไบต์
→ บน A53 คาดว่า `lut` จะลด 49.7 → **~4–8 ms** ได้โดย**ไม่ต้องแตะ PL เลย**

ดังนั้นการวัด M13 ต้องรายงาน **3 จุด** ไม่ใช่ 2: `numpy` (baseline เดิม) → `lut` (SW ที่ optimize แล้ว) → `hw` (PL)
ค่าที่ HW ต้องชนะจริงคือ `lut` ไม่ใช่ `numpy` — ข้อดีของ HW ที่เหลืออยู่คือ (1) ปลด CPU ไปทำ decode/track/IO
(2) latency ไม่ขึ้นกับขนาดเฟรมต้นทาง (1080p ก็ ~5 ms) (3) ต่อ pipeline กับ MIPI/ISP ใน PL ได้ในอนาคต
